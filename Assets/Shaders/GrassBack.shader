...
SubShader
	...
	Tags
	{
		...
		HLSLINCLUDE
			...
			#pragma multi_compile _ _SCREEN_SPACE_OCCLUSION   // needed?
		...
		Pass
		{
			...
			HLSLPROGRAM
				...
				float3 lightContribution(Light l,SurfaceVariables s,float mask){
					...
					float attenuation = l.distanceAttenuation * smoothstep(0,mask,l.shadowAttenuation);
					// float3 shading = l.color.rgb * diffuse;
					float3 shading = l.color.rgb * smoothstep(0.1,0.5,diffuse);
				...
				float4 frag(GeomData i) : SV_Target
				{
					...
					float3 tangentNormal = UnpackNormal(tex2D(_GroundNormal, TRANSFORM_TEX(i.uv, _GroundDiffuse)));
					float3x3 tbn = CreateTangentToWorld(i.normal, i.tangent,0);
					...
					s.normal = lerp(normalWS,i.normal,min(max(mask*2,0),1));
...
// Main directional light contribution
					...
					float3 lighting = lightContribution(light,s,smoothstep(0,1,0.5+0.5*max(_GrassThreshold-mask,0))); //*SampleAmbientOcclusion()
				    ...
				    // 3) Fetch the occlusion factor (1 = fully lit, 0 = fully occluded)
				    float ao = 1-SampleAmbientOcclusion(screenUV);
					ao = pow(ao,2);
					ao = 1-min(ao,1);
					// litColor*=ao;

					float3 finalColor = quantize(litColor)*litColor + quantize(ao)*_Ambient*baseColor;
					// finalColor*=quantize(ao);
